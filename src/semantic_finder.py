...
class SemanticWordFinder:
        ...
        try:
            if os.path.exists(self.cache_file):
                with open(self.cache_file, "r") as f:
                    ...
                    self.embeddings_cache = {word: np.array(embedding) for word, embedding in cache_data.items()}
    ...
    def _save_embeddings_cache(self):
        ...
        try:
            cache_data = {word: embedding.tolist() for word, embedding in self.embeddings_cache.items()}
    ...
    def semantic_merge(
        self, word1_embedding: np.ndarray, word2_embedding: np.ndarray, alpha: float = 0.5
    ) -> np.ndarray:
        """
    ...
    def find_best_combinations(
        self, available_words: List[str], target_word: str, top_k: int = 5, test_alphas: bool = False
    ) -> List[Dict]:
        """
    ...
    def _fallback_heuristics(self, available_words: List[str], target_word: str, top_k: int) -> List[Dict]:
        ...
        for word1, word2 in combinations(available_words, 2):
            ...
            combinations_scores.append(
                {"word1": word1, "word2": word2, "score": score, "alpha": 0.5, "confidence": "heuristic"}
            )
