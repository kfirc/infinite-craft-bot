...
class TargetWordAutomation(AutomationController):
    ...
    def __init__(self, strategy_config: Dict = None, log_level: str = "INFO"):
        ...
        self.log("INFO", f"🎯 TARGET WORD HUNTER INITIALIZED")
        ...
        self.log("INFO", f"   Semantic model: {'✅ Available' if self.semantic_finder.is_model_available() else '❌ Using heuristics'}")
    ...
    def run_target_hunt_session(self) -> Dict:
        ...
        """
        self.log("INFO", f"🚀 Starting Target Hunt Session")
        self.log("INFO", f"🎯 Strategy: target_word_hunter")
        self.log("INFO", f"🎪 Target: {self.target_word}")
        self.log("INFO", "=" * 60)
        ...
        try:
            ...
            while self.attempts_made < self.max_attempts:
                ...
                self.log("INFO", f"🧠 Finding best semantic combinations...")
                ...
                if not best_combinations:
                    self.log("WARNING", f"🛑 All good combinations already tested - ending hunt")
                ...
                for i, combo in enumerate(best_combinations, 1):
                    ...
                    # CRITICAL: Try combination with shared retry mechanism - don't mark as tried until success/permanent failure
                    ...
                    if self._check_target_found():
                        target_element = self._find_target_in_sidebar()
                        self.log("INFO", f"🎉 TARGET FOUND (indirect): {target_element['name']} {target_element['emoji']}")
                        session_results.update({
                            'target_found': True,
                            'target_element': target_element,
                            'hunt_success': True
                ...
                self.log("INFO", f"📊 Iteration {iteration} complete - Discovered: {len(self.discovered_elements)} new elements total")
    ...
    def _check_target_found(self) -> bool:
        ...
        self.automation.update_sidebar_cache()
        return any(
            elem['name'].lower() == self.target_word.lower() 
    ...
    def run_complete_automation(self) -> bool:
        ...
        try:
            ...
            if results['hunt_success']:
                ...
                self.log("INFO", f"⏳ Hunt failed - '{self.target_word}' not found in {results['attempts_made']} attempts")
...
def hunt_specific_word(target_word: str, max_attempts: int = 50) -> bool:
    """
    Convenience function to hunt for a specific word using the generic controller pattern.
...
def main():
    """Main entry point for target word automation."""
    import sys
